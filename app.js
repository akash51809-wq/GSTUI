...
function buildUploadWorkspace(){
  ...
  style.textContent=`
    #upload .upload-workspace{display:grid;grid-template-columns:minmax(300px,.92fr) minmax(420px,1.28fr);gap:18px;margin-top:4px}
    #upload .upload-hero-card,#upload .workflow-card,#upload .uploaded-card{background:rgba(255,255,255,.97);border:1px solid #E2E8F0;border-radius:24px;box-shadow:12px 15px 28px rgba(15,23,42,.1),-5px -5px 14px rgba(255,255,255,.92),inset 1px 1px 2px rgba(255,255,255,.95);position:relative;overflow:hidden}
    #upload .upload-hero-card{padding:24px;min-height:300px;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;background:linear-gradient(145deg,#fff,#F4F8FC)}
    #upload .upload-hero-card:before{content:"";position:absolute;width:250px;height:250px;border-radius:50%;right:-145px;top:-145px;background:radial-gradient(circle,rgba(37,99,235,.14),transparent 68%);pointer-events:none}
    #upload .upload-hero-card:after{content:"";position:absolute;width:190px;height:190px;border-radius:50%;left:-120px;bottom:-125px;background:radial-gradient(circle,rgba(13,148,136,.13),transparent 68%);pointer-events:none}
    #upload .dropzone-3d{width:100%;min-height:245px;border:1.5px dashed #93C5FD;border-radius:21px;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:20px;background:linear-gradient(145deg,rgba(239,246,255,.9),rgba(248,250,252,.92));box-shadow:inset 3px 3px 9px rgba(37,99,235,.05),inset -3px -3px 9px rgba(255,255,255,.9);position:relative;overflow:hidden;transition:.25s}
    ...
    #upload .upload-orb-3d{width:82px;height:82px;border-radius:27px;display:grid;place-items:center;background:linear-gradient(145deg,#60A5FA,#1E3A8A);color:#fff;font-size:37px;font-weight:950;box-shadow:11px 13px 23px rgba(30,58,138,.24),inset 2px 2px 4px rgba(255,255,255,.38),inset -3px -3px 6px rgba(15,23,42,.2);transform:rotate(-7deg);animation:uploadFloat 2.4s ease-in-out infinite;position:relative;z-index:1}
    #upload .dropzone-3d h3{font-size:17px;margin:16px 0 5px;color:#0F172A;position:relative;z-index:1}
    #upload .dropzone-3d p{font-size:10px;color:#64748B;margin:0 0 15px;position:relative;z-index:1}
    #upload .upload-choose-btn{border:0;background:linear-gradient(145deg,#3B82F6,#1E3A8A);color:#fff;border-radius:12px;padding:11px 17px;font-size:10px;font-weight:900;cursor:pointer;box-shadow:7px 8px 15px rgba(37,99,235,.22),inset 1px 1px 2px rgba(255,255,255,.3);position:relative;z-index:1;transition:.2s}
    #upload .upload-choose-btn:hover{transform:translateY(-3px);box-shadow:10px 13px 20px rgba(37,99,235,.27)}
    #upload .upload-limit{font-size:8px;color:#94A3B8;margin-top:12px;position:relative;z-index:1}
    #upload .upload-progress-wrap{width:100%;margin-top:17px;position:relative;z-index:1}
    #upload .upload-progress-track{height:8px;border-radius:99px;background:#E2E8F0;overflow:hidden;box-shadow:inset 2px 2px 4px rgba(15,23,42,.08)}
    ...
    #upload .upload-progress-label{display:flex;justify-content:space-between;font-size:8px;font-weight:800;color:#64748B;margin-top:7px}
    #upload .process-main-btn{width:100%;margin-top:12px;border:1px solid rgba(16,185,129,.3);border-radius:13px;padding:12px 16px;color:#fff;background:linear-gradient(145deg,#10B981,#047857);font-size:10px;font-weight:950;letter-spacing:.3px;cursor:pointer;box-shadow:8px 9px 17px rgba(5,150,105,.2),inset 1px 1px 2px rgba(255,255,255,.3);position:relative;overflow:hidden;transition:.2s}
    ...
    #upload .process-main-btn:hover{transform:translateY(-3px);box-shadow:11px 13px 22px rgba(5,150,105,.25)}
    #upload .process-main-btn span{position:relative;z-index:1}
    #upload .workflow-card{padding:22px;min-height:300px;background:linear-gradient(145deg,#FFFFFF,#F8FAFC)}
    #upload .workflow-head{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin-bottom:16px}
    #upload .workflow-title{font-family:"Trebuchet MS","Segoe UI",sans-serif;font-size:18px;font-weight:950;letter-spacing:1.2px;background:linear-gradient(90deg,#1E3A8A,#2563EB,#0D9488,#1E3A8A);background-size:240% auto;-webkit-background-clip:text;background-clip:text;-webkit-text-fill-color:transparent;animation:workflowGradient 3.8s linear infinite,workflowFloat 2.2s ease-in-out infinite;margin:0}
    #upload .workflow-subtitle{font-size:9px;color:#64748B;margin-top:5px}
    #upload .workflow-live{padding:6px 9px;border-radius:10px;background:#ECFDF5;color:#047857;font-size:8px;font-weight:900;white-space:nowrap;box-shadow:inset 1px 1px 2px #fff,3px 4px 8px rgba(5,150,105,.08)}
    #upload .workflow-list{display:flex;flex-direction:column;gap:7px}
    #upload .workflow-step{display:grid;grid-template-columns:29px 1fr auto;align-items:center;gap:10px;padding:9px 10px;border-radius:13px;border:1px solid transparent;transition:.25s;position:relative}
    #upload .workflow-step:after{content:"";position:absolute;left:23px;top:38px;width:2px;height:17px;background:#E2E8F0}
    ...
    #upload .step-number{width:29px;height:29px;border-radius:9px;display:grid;place-items:center;background:linear-gradient(145deg,#E2E8F0,#CBD5E1);color:#64748B;font-size:8px;font-weight:950;box-shadow:3px 4px 7px rgba(15,23,42,.08),inset 1px 1px 2px #fff}
    ...
    #upload .step-copy strong{display:block;font-size:10px;color:#0F172A}.step-copy small{display:block;font-size:7px;color:#94A3B8;margin-top:2px}
    #upload .step-status{font-size:7px;font-weight:900;color:#94A3B8}.workflow-step.active .step-status{color:#2563EB}.workflow-step.done .step-status{color:#059669}
    #upload .uploaded-card{grid-column:1/-1;padding:20px;min-height:205px}
    #upload .uploaded-head{display:flex;justify-content:space-between;align-items:center;margin-bottom:12px}.uploaded-head h3{margin:0;font-size:14px}.uploaded-count{font-size:8px;color:#64748B;background:#F1F5F9;padding:6px 9px;border-radius:9px;font-weight:800}
    #upload .invoice-list{display:flex;flex-direction:column;gap:8px;max-height:300px;overflow:auto}
    #upload .invoice-row{display:grid;grid-template-columns:34px minmax(130px,1.2fr) minmax(100px,.8fr) 90px 100px 90px;align-items:center;gap:10px;padding:10px 11px;border:1px solid #E2E8F0;border-radius:13px;background:linear-gradient(145deg,#fff,#F8FAFC);box-shadow:4px 6px 12px rgba(15,23,42,.06),inset 1px 1px 2px #fff;animation:rowIn .35s ease both}
    #upload .pdf-badge{width:32px;height:32px;border-radius:10px;display:grid;place-items:center;background:linear-gradient(145deg,#FCA5A5,#EF4444);color:#fff;font-size:9px;font-weight:950;box-shadow:4px 5px 8px rgba(239,68,68,.16)}
    #upload .invoice-file strong,#upload .invoice-file small{display:block;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.invoice-file strong{font-size:9px;color:#0F172A}.invoice-file small{font-size:7px;color:#94A3B8;margin-top:3px}.invoice-meta{font-size:8px;color:#475569}.invoice-amount{font-size:9px;font-weight:900;color:#0F172A}.invoice-status{font-size:8px;font-weight:900;padding:5px 7px;border-radius:8px;text-align:center;background:#EFF6FF;color:#2563EB}.invoice-status.done{background:#ECFDF5;color:#047857}.invoice-status.warn{background:#FFFBEB;color:#B45309}
    ...
    @media(max-width:900px){#upload .upload-workspace{grid-template-columns:1fr}.invoice-row{grid-template-columns:34px 1fr 80px}.invoice-row>:nth-child(3),.invoice-row>:nth-child(5){display:none}}
    @media(max-width:560px){#upload .upload-hero-card,#upload .workflow-card,#upload .uploaded-card{padding:14px}.invoice-row{grid-template-columns:32px 1fr 72px}.email-toast{right:14px!important;left:14px;width:auto!important;bottom:18px!important}}
  `;
  document.head.appendChild(style);
  ...
  section.innerHTML=`
    ...
    <div class="upload-workspace">
      ...
      <div class="uploaded-card">
        ...
        <div class="invoice-list" id="invoiceList"><div style="padding:28px;text-align:center;color:#94A3B8;font-size:9px">Uploaded invoices will appear here automatically.</div></div>
      </div>
    </div>`;
  ...
  function renderFiles(){
    ...
    if(!files.length){list.innerHTML='<div style="padding:28px;text-align:center;color:#94A3B8;font-size:9px">Uploaded invoices will appear here automatically.</div>';return}
