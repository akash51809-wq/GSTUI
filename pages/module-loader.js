...
(function(){
  ...
  const reportSubpages={
    ...
    'upload-invoice-report':{html:'pages/report/upload-invoice-report.html',css:'pages/report/upload-invoice.css',js:'pages/report/upload-invoice-report.js'},
  ...
  function loadJs(u){
    return new Promise((resolve,reject)=>{
      ...
      const s=document.createElement('script');s.src=v;s.dataset.gstuiModuleJs=v;s.onload=resolve;s.onerror=()=>reject(new Error('Failed to load '+u));document.body.appendChild(s);
    });
  }
  ...
  async function mount(name){
    ...
    await loadInto(host,meta,name);if(name==='reports')await mountReportSubpages();
  }
  async function mountAll(){
    for(const name of Object.keys(modules)){try{await mount(name)}catch(e){console.error('[GSTUI module-loader]',e)}}
