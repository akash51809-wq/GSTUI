...
function findElement(html, id) {
  ...
  while ((token = tokenRe.exec(html))) {
    ...
    if (depth === 0) return { start, end: tokenRe.lastIndex, tag, content: html.slice(start, tokenRe.lastIndex) };
  }
...
for (const [id, meta] of Object.entries(pageMap)) {
  ...
  writeIfMissing(path.join(dir, meta.css), `/* ${id} page-specific styles. Shared theme/layout remains in ../../style.css. */\n`);
  writeIfMissing(path.join(dir, meta.js), `/* ${id} page-specific behavior. Common navigation remains in ../../app.js. */\n`);
}

// Keep the Report subpages available as independent modules too.
for (const id of ['pending-invoice', 'upload-invoice-report', 'ai-report']) {
  ...
  if (!fs.existsSync(htmlFile) || fs.readFileSync(htmlFile, 'utf8').includes('module placeholder')) fs.writeFileSync(htmlFile, found.content, 'utf8');
...
// Ensure the loader is present before app.js so page hosts are populated before navigation.
